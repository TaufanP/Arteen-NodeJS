...
module.exports = {
    ...
    pageProduct: (pages)=>{
        return new Promise((resolve, reject)=>{
            ...
            connection.query("SELECT * FROM product_name  LIMIT 2 OFFSET ?", startIndex,(err, result)=>{
                if(!err) {resolve(result);}
                else{reject(new Error(err));}
            });
        });
    ...
    productDetail: (id_product)=>{
        return new Promise((resolve, reject)=>{
            connection.query(`SELECT p.id, p.name, p.description, p.price, p.stock, p.image, c.name_category AS category, p.updated_at, p.created_at
            FROM product_name p
            INNER JOIN category c ON p.id_category = c.id WHERE p.id = ${id_product}`,(err, result)=>{
                if(!err) {resolve(result);}
                else{reject(new Error(err));}
            });
        });
    ...
    searchProduct: (keyword)=>{
        return new Promise((resolve, reject)=>{
            connection.query("SELECT * FROM product_name WHERE name LIKE ?", '%' + keyword + '%', (err, result)=>{
                if(!err) {resolve(result);}
                else{reject(new Error(err));}
            });
        });
    ...
    sortProduct: (susun2) =>{
        return new Promise((resolve, reject) => {
            connection.query(`SELECT * FROM product_name ORDER BY product_name.${susun2}`, (err, result) => {
                if(!err){resolve(result)}
                else{reject(new Error(err));}
            });
        });
